var LocalStorageAdapter = function () {
    ...
    this.initialize = function() {
        ...
        window.localStorage.setItem("Resturants", JSON.stringify(
            [
                {"id": 1, "Resturant": "Chiptole", "pic": "chiptole-logo.jpg","email":"testUser1@test.com","location":null},
                {"id":2,"Resturant": "Jimmy John's","pic":"jimmy-johns-logo.jpg","email":"testUser2@test.com","location":null}
            ]
        ));
        deferred.resolve();
    ...
    this.addUser = function(Resturant,email,location){
        var deferred = $.Deferred();
        Resturants = JSON.parse(window.localStorage.getItem("Resturants")),
        Resturant = null,
        l = Resturants.length;
        window.localStorage.setItem("Resturants",JSON.stringify(
            [
                {"id":(l+1), "Resturant":Resturant,"pic":null,"email":email,"location":location}
            ]
        ));
        deferred.resolve();
        return deferred.promise();
    ...
    this.findByName = function (searchKey) {
        var deferred = $.Deferred(),
            Resturants = JSON.parse(window.localStorage.getItem("Resturants")),
            results = Resturants.filter(function (element) {
                var name = element.Resturant;
